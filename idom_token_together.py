...
from contrastive_model_with_idiom import BertForChID
...
class DataTrainingArguments:
    ...
    idiom_file: Optional[str] = field(default=None, metadata={"help": "The input idiom knowledge data file (a text file)."})
...
def main():
    ...
    if data_args.train_file is not None or data_args.validation_file is not None:
        ...
        if data_args.idiom_file is not None:
            idiom_files["train"] = data_args.idiom_file
        ...
        idiom_datasets = load_dataset(
            extension,
            data_files=idiom_files,
            cache_dir=model_args.cache_dir,
            use_auth_token=True if model_args.use_auth_token else None,
        )
    else:
    ...
    single_word_embedding = model.bert.get_input_embeddings()
    new_word_embedding = nn.Embedding(52023, 768, padding_idx=0)
    nn.init.normal_(new_word_embedding.weight, std=0.05)
    with torch.no_grad():
        new_word_embedding.weight[:21128] = copy.deepcopy(single_word_embedding.weight) 
    model.bert.set_input_embeddings(new_word_embedding)
    
    # state_dict = torch.load("./tmp/checkpoint-57940", map_location="cpu")
    ...
    idiom_data = idiom_datasets["train"]
    ...
    def preprocess_function_tokenize(examples): 
        ...
        idiom_indexs = [idx-21128 for candidate_ids in tokenized_candidates for idx in candidate_ids]
        candidates_explains = idiom_data[idiom_indexs]["explaination"]
        # for candidate in candidates_explains:
        # import pdb
        # pdb.set_trace()
        tokenized_explains = tokenizer(
            candidates_explains,
            max_length=60,
            padding="max_length",
            truncation=True,
        ...
        # import pdb
        # pdb.set_trace()
        # for key in tokenized_explains.keys():
        #     tokenized_explains[key] = list(np.array(tokenized_explains[key]).reshape(-1,7))
        # print("lenth:", len(tokenized_explains["input_ids"]))
        # if len(tokenized_explains["input_ids"])!=7000:
            
        explainations_input_ids = np.array(tokenized_explains["input_ids"]).reshape(-1,7,60)
        explainations_attention_mask = np.array(tokenized_explains["attention_mask"]).reshape(-1,7,60)
        explainations_token_type_ids = np.array(tokenized_explains["token_type_ids"]).reshape(-1,7,60)
        # print(explainations_input_ids.shape)
        tokenized_examples["explainations_input_ids"] = explainations_input_ids
        tokenized_examples["explainations_attention_mask"] = explainations_attention_mask
        tokenized_examples["explainations_token_type_ids"] = explainations_token_type_ids
