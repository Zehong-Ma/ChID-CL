...
from embedding_model import BertForChID
...
def main():
    ...
    def preprocess_function_resize(examples):
        return_dic = {}
        return_dic_keys = ['explaination', 'labels', "idiom_ids"]
        for k in return_dic_keys:
            return_dic[k] = []

        for i in range(len(examples['explaination'])):
            idx = -1
            text = examples['explaination'][i]
            
            idx = text.find(idiom_tag, idx+1)
            return_dic['explaination'].append(text[:idx] + tokenizer.mask_token + text[idx+len(idiom_tag):])
            return_dic['idiom_ids'].append(examples['id'][i])
            return_dic['labels'].append(0)
        return return_dic
    ...
    def preprocess_function_tokenize(examples): 
        first_sentences = examples['explaination']
        ids = examples["id"]
        ...
        tokenized_examples["idiom_ids"] = ids 
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            train_dataset = train_dataset.map(
                ...
                batched=True,
                remove_columns=["idiom", "example", "derivation"],
    ...
    torch.save({"idiom_embeddings": model.bert.embeddings.word_embeddings}, "./data/idiom_word_embedding.pt")
    # kwargs = dict(
